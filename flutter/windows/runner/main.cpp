...
{
  HINSTANCE hInstance = LoadLibraryA("libselfdesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libselfdesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  {
    std::cout << "Failed to get selfdesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  std::wstring app_name = L"SelfDesk";
